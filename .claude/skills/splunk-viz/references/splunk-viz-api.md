...
- **⚠️「アップグレードしても旧UIのまま」の第一容疑者はリバースプロキシのキャッシュ**（2026-07-22 確定）:
  ユーザーの Splunk は **Cloudflare トンネル経由**でもアクセスされており、トンネル経由だと
  アップグレード＋_bump＋ハードリロードをしても旧 visualization.js が配信され続けた。
  **直接 IP アクセスでは反映済み**だったことで確定（link-line v1.1.0/1.2.0 で実発生）。
  反映確認はまず直 IP で行い、トンネル側は Cloudflare のキャッシュパージで対処する。
  （調査過程で package.mjs の build 番号エポック秒化も試したが原因ではなくユーザー指示で取り消し済み。
  buildNumber はコミットハッシュ由来のまま）
- バンドル内の日本語は esbuild が `線` 形式（大文字hex）でエスケープするため、
  **日本語文字列の grep では新旧バンドルを判定できない**（ASCII の識別子や `VIZ_VERSION` 文字列で
  判定する。viz 側にバージョン定数を入れて UI/debug に出しておくと実機での反映確認が一目でできる。
  link-line v1.2.0 で採用）。
...
### ⚠️ 編集モード中は viz 内のマウス操作 UI が効かない（iframe への入力遮断）

Studio 編集モードのイベント設計（dashboard-layouts/core/ui v29.8 で確認）:
- パネル移動＝ホバー時に上部中央へ出るピル型ドラッグハンドル（88×36px, `useDragHandleComponent`,
  `EVENT_MOUSE_DOWN_ON_HANDLE` でのみ `isMovingRef=true`）から。
- **viz 本体への mousedown はパネル選択に消費**される（`SelectableContainer.onMouseDown` →
  `EVENT_MOUSE_DOWN_ON_VIZ_WITH_HANDLE`）。これを iframe 拡張 viz でも成立させるため、
  編集モード中はホストが拡張 iframe への入力を遮断する（実機 link-line で「viz 全体に判定が
  吸われる」を観測。遮断方式=pointer-events 無効化 or 透明カバーの特定は devtools で）。
- → **編集モード中に viz 内へ描いたクリック/ドラッグ UI（アイコンピッカー・編集ハンドル等）は
  実機では動かない**。kpi-tile の編集モードアイコンクリックも同罠の可能性大（実機未確認だった）。
- 対策（link-line v1.3.0 で確立・**表示モードの setOptions はホスト定義に載らないことが実機確定**）:
  「**表示モードで操作 → 変更を viz 内 pending に保持 → mode が edit に変わった瞬間に setOptions を
  ...
  表示モード中の見た目はローカル draft でライブプレビューする（ホストが echo を返さないため）。
  参照実装: link-line の `pendingRef` / flush effect。確実な保険は右パネル（editor.number 等）完結の操作。
...
- **viz 内に「動的色設定」風パネルを自前実装**（link-line v1.2.0 で確立。参照実装:
  `custom-viz-link-line/.../visualization.jsx` の `parseColorBands`/色設定パネル）。
  範囲リスト（＋追加/×削除/プリセット/⇅反転）を `<input type="color">`+`<input type="number">` で
  組み、JSON文字列オプション（例 `colorBands`）へ `setOptions` 保存。**編集モードは iframe への
  入力遮断があるため、パネルは表示モードに置く**（§編集モードの入力遮断 参照）。
