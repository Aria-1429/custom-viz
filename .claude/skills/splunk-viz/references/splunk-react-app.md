...
> **⭐ この方式の上に「独自ダッシュボード基盤」を載せたものが DPX**（`apps/dash-platform/`）。
> ページ1枚を手で作り込むのではなく、**JSON 定義から任意のダッシュボードを描画・GUI 編集する**
> なら [dpx-platform.md](dpx-platform.md) を読む（2026-08-10 構築・実機検証済み）。
> 本ファイルの知見（Mako テンプレート・`useSearch`・ヘッダ非表示・全画面の注意点）は
> DPX の土台としてそのまま生きている。
