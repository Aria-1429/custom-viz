...
| `server_infra_monitoring_dashboard.json` | 44 | **サーバインフラ監視（3層Webシステム）**（2026-08-08）。Icon Status ＋ Link Line で LB → Web → App → DB ＋キャッシュ／バックアップ分岐の系統図を組む。`network_topology` と同じ手法だがサーバ主体で、**LB から2本に分岐 → APP 2台が DB に合流**という分岐・合流のある形。**16機器・17リンク**の系統図（INTERNET→FW→LB×2→WEB×3→APP×2→DB＋キャッシュ/認証/レプリカ/バックアップ、IDS→SIEM の監視系統も）。**線のパネルを機器に重ねて置き、端点をアイコン境界でトリムしてある**（→ §4.2）。生成は `tools/dashboard-loop/gen-topology-map.py` |
...
> 重ねてよいので線を「アイコン中心→アイコン中心」の長さで置け、ラベルも見切れない。
> ただし**線がアイコンの絵の上を横切るのは NG** なので、端点をアイコン境界で止める。
> アイコンの実寸・トリム量は §4.2 を参照。`tools/dashboard-loop/gen-topology-map.py`
> に実装がある（`server_infra_monitoring_dashboard.json` はこれで生成）。
...
## 4.1 ⚠ 撮影時の「データがありません」は実機の不具合とは限らない（2026-08-08 実機で確定）

**パネル数が多いダッシュボード（27枚）を `sync.mjs` / `shot.mjs` で撮ると、
毎回 0〜3 枚が「データがありません。サーチ結果を確認してください。」になる。
しかも撮り直すたびに空になるパネルの顔ぶれが変わる。**

### 事実として確認できたこと

- **サーチ側は正常**。撮影直後に `services/search/jobs` を見ると
  **全ジョブが `DONE`、`messages` は空、QUEUED は 0 件**
- **`spl-check.mjs` は全件 ✓**
- 空になったパネルを**個別に撮り直すと普通に描画される**回がある
- `--wait` を 150 秒まで伸ばしても解消しない
- **同じ JSON・同じ実機で、空 0 枚の回と 3 枚の回がある**

→ **ヘッドレス撮影時の描画タイミングの問題**であって、ブラウザで開けば全部描画される。
**スクショだけ見て「このパネルは壊れている」と判断しない。**
判断するなら「複数回撮って毎回同じパネルが空か」「ジョブが DONE か」を見る。

### 【訂正】「同時サーチ数の上限が原因」は誤りだった

最初、実機の `base_max_searches = 6` / `max_searches_per_cpu = 1` を見て
「`ds.search` が多いと starvation する」と考え、`ds.chain` で 12 本 → 5 本まで減らした。
**が、5 本でも空パネルは出た**し、**チェーン側（自前のサーチを投げないはず）のパネルまで空になった**。
ジョブを見たら**キューは常に空で全件 DONE** だったので、この説明は成り立たない。

**教訓**：設定値（`base_max_searches`）を見ただけで因果を組み立て、
**実際にキューが詰まっているかを確認する前に断定した**のが誤り。
`services/search/jobs` の `dispatchState` を見れば1回で否定できた。

### それでも `ds.chain` で基底サーチを減らすのは正しい

同じ静的 `makeresults` を何本も走らせる意味は無い（27 パネルで `ds.search` 7 本にした）。

```jsonc
"ds_static":  { "type": "ds.search", "options": { "query": "| makeresults format=csv data=\"kind,c1,c2\nnode,WEB-01,48\nnode,DB-PRIMARY,88\"" } },
"ds_node_db": { "type": "ds.chain",  "options": { "extend": "ds_static", "query": "| where kind=\"node\" AND c1=\"DB-PRIMARY\" | eval label=c1, cpu=tonumber(c2) | table label cpu" } }
```

- ⚠ **絞り込みは `| where`。`| search` だと 0 行になる**
  （`makeresults` 由来のフィールドには効かない。実機確認済み）
- ⚠ **`spl-check.mjs` は `extend` を解決しない**ので、チェーンした dataSource は
  全部「0 行」と報告される。**ツールの限界であって不具合ではない**。
  検算するなら親を再帰的に連結してから oneshot で流す

---

## 4.2 Icon Status × Link Line で系統図を組む（2026-08-08 実機で確定）

### アイコンはパネルの中央には無い

値とラベルの文字がアイコンの**下**に積まれるので、アイコンは**上寄り**になる。
`168x168` / `showCard:false` / `iconScale:1` のとき（`visualization.jsx` から導出）:

| 量 | 値 |
|---|---|
| アイコン上端 | y = 3 |
| アイコン下端 | y = 104 |
| **アイコン中心** | **y = 53.5 ＝ パネル高の 0.318**（0.5 ではない） |
| アイコン半径 | 50.5 |
| 値＋ラベルの文字 | y = 106 〜 163 |

**中心を 0.5 と決め打ちすると線がアイコンからずれる。**

### 線はアイコンの境界で止める（重ねてよいが、絵の上は通さない）

**パネルの前後関係（z-order）はダッシュボード JSON からは制御できない**ので、
「線を背面へ回す」ことはできない。**端点を手前で止める**のが確実:

```python
TRIM_SIDE = 50.5 + 6    # 左右：アイコン半径＋余白
TRIM_UP   = 50.5 + 6    # 上へ抜ける：アイコンだけ避ければよい
TRIM_DOWN = 118         # 下へ抜ける：アイコン＋値＋ラベルを避ける
```

⚠ **上下で非対称**にすること。均一にすると**下向きの線だけが数値の上を通る**
（FW-01 の「46%」、DB-REP の「61%」で実際に発生）。

### 線のパネルサイズ

- **縦向きの線はパネルを横に広く**取る（ラベルのチップが横に出る）。
  幅 68px では「ミラー TAP」「レプリカ 33%」が見切れた → **264px で解消**
- **端点の余白**：丸キャップは `capR*1.9`（`capR = clamp(lw*0.8+3, 6, 24)`)、
  矢印は `max(11, lw*2.3)`。`linePoints` を `0.02` のような端に置くと
  **始点の丸キャップが半分に切れる**（実機で発生）
- **`styleMode: "shadow"` は暗い背景でほぼ見えない**（lw=5）。`neon` lw=6 にする

### ラベルの位置は `labelPos` で動かせる（link-line v1.11.0〜）

**値ラベルの既定位置は「線の中点」**なので、折れ線だと**中点がノードの数字の上に来る**ことがある。
`labelPos`（**0〜1 のパネル相対座標**を JSON 文字列で）で任意の位置に置ける:

```jsonc
"options": {
  "linePoints": "[[0.25,0.25],[0.5,0.25],[0.5,0.75],[0.75,0.75]]",
  "labelPos":   "[0.5, 0.5]"     // [x, y]。{"x":…,"y":…} 形式も可
}
```

- **`config.json` の `optionsSchema` には無いキー**だが、**定義に保存され viz に届く**
  （→ [[options-key-without-schema]]）。**splunkd の再起動は要らない**
- 空文字／不正な JSON なら中点追従に戻る
- 表示画面の「✎ 線を編集」でドラッグしても設定できる（ダブルクリックで中央に戻る）

**置き場所の決め方**（2026-08-08 に 17 本で実施）：ラベルは**ノードの列と列の隙間**に置く。
チップの幅はだいたい `34 + ラベル文字数*11 + 値の文字数*8` px なので、
**列の間隔をチップ幅より広く取る**（この系統図では隙間 132px にして「アラート転送 OK」= 約124px を収めた）。
中点のままだと衝突するのは以下のようなケース:

| 症状 | 直し方 |
|---|---|
| 折れ線の中点がノードの数値の上に乗る | 縦の脚ではなく**横の脚**へ、または列間の隙間へ移す |
| 兄弟リンクのチップ同士が数 px で重なる | 片方を自分の脚に沿ってずらす |
| チップがアイコンのグローに掛かる | `x` を 0.5 から少しずらす（0.44〜0.46 程度） |

⚠ **「縦の脚の 0.5」は安全ではない**。パネルが縦に長いと 0.5 がちょうどノードの上に来る。
**絶対座標で空きを確認してから相対値に直す**こと（実際に1度これで悪化させた）。

### 重なり検証の読み替え

線と機器を重ねる構成では `validate-dashboard.mjs` の「重なり」が大量に出る（74 件）。
**link×node の重なりは意図的**なので、下記で分類してから判断する:

```bash
node tools/dashboard-loop/src/validate-dashboard.mjs <file> | grep "✗ 重なり" \
  | grep -vE "(link_.* と viz_node_|node_.* と viz_link_)"   # ← これが空ならOK
```
**link×link と node×node は 0 件でなければならない**（ラベルが重なる／機器が潰れる）。
...
#### 撮影を速くする（2026-08-09 実測。29 パネルのダッシュボードで計測）

**`--wait` の単位は「秒」。しかも「各パネルの待ち時間」ではなく「画面が安定するまでの上限」**。
`waitForRender()` は**毎秒フルスクリーンショットを撮って前回と bit 比較**し、
2秒同一なら完了とみなす。ここを取り違えて `--wait 25000` と書いたら
**上限が約7時間**になり、アニメーションする viz のせいで永久に打ち切られず走り続けた（実害）。

**アニメーションする viz が1枚でもあると画面は絶対に安定しない**（attack-globe /
liquid-tube / network-graph 等）。つまりそういうダッシュボードでは
**`--wait` は「必ず消費する固定コスト」**になる。`settled: false` で終わるのが正常。

実測（29 パネル・1920x4328）:

| 指定 | 所要 | 空表示パネル |
|---|---|---|
| 既定（`--wait 75 --scale 2`） | 107 秒 | 8 |
| `--wait 25 --scale 2` | 52 秒 | 3 |
| `--wait 12 --scale 2` | 54 秒 | 6 |
| **`--wait 25 --scale 1`** | **39 秒** | **1** ← 最良 |
| `--wait 12 --scale 1` | 27 秒 | 7 |

- **⭐ `--scale 1` が最も効く**。2x だと待機中の毎秒スクショまで巨大化して
  ブラウザを圧迫し、**遅いうえに空表示が増える**（8枚 → 1枚に激減した）。
  細部を読みたいときだけ 2x にし、**普段は `--scale 1`** でよい。
- **`--wait` を削りすぎると逆効果**。12 秒では所要がほぼ変わらないのに空表示が増える
  （＝サーチ完了前に撮って誤診しやすくなる）。**25 秒前後が下限の目安**。
- 「短くして速く回す」より「誤診しない」が優先なのは変わらない。
  **速くしたいならまず `--scale 1`、それでも足りなければ `--wait` を 25 秒まで**。
