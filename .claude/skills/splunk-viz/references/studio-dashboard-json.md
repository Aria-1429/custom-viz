...
1. **インポート手順**：Dashboard Studio で新規ダッシュボード作成 →「ソースコードを編集」に全文貼り付け
2. **前提**：必要な viz アプリの一覧（未導入だとそのパネルだけ落ちる）
3. **手動設定が要る箇所**：トークン設定（インタラクション）など
4. **未検証の点**：WebGL パネルの同時配置数など、実機でしか分からないもの
