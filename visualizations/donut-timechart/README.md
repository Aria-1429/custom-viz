...
- 生成した `.spl`: `dist/custom_viz_donut_timechart-1.1.5-075cf5f.spl`
...
- **編集パネルのチェックボックスのラベルを短縮**した。長いラベルは編集パネルで
  ...
  | 旧 | 新 |
  |---|---|
  | 中央に最大セグメントの割合を表示 | 中央に割合を表示 |
  | パネルサイズに自動フィット | 自動フィット |

  **オプションのキー名・既定値・挙動は変更していない**（表示ラベルのみ）。
  ⚠ 編集パネルへの反映には splunkd の再起動が必要。

`.spl`: `dist/custom_viz_donut_timechart-1.1.4-<hash>.spl`
...
`.spl`: `dist/custom_viz_donut_timechart-1.1.3-2bb5ec3.spl`
