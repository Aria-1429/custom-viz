...
- 生成した `.spl`: `dist/custom_viz_country_graph-1.1.5-075cf5f.spl`
...
`.spl`: `dist/custom_viz_country_graph-1.1.4-2bb5ec3.spl`
...
- **表示名から `Custom Viz ` プレフィックスを削除**（`Custom Viz Country Graph` → `Country Graph`）。
...
- `dist/custom_viz_country_graph-1.1.1-b4e4674.spl`
...
- **`debug`（オプション診断オーバーレイ）を廃止**した。`config.json` の `optionsSchema` /
  ...
  `normalizeOptions` / オーバーレイ描画をすべて削除した。 オーバーレイ専用のスタイル定義 `debugBox` も併せて削除した。

#### 成果物

- `dist/custom_viz_country_graph-1.1.0-fcde869.spl`
...
- `dist/custom_viz_country_graph-1.0.2-2dc2dde.spl`
