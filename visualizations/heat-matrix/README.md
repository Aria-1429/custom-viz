...
- **編集パネルのチェックボックスのラベルを短縮**した。長いラベルは編集パネルで
  ...
  | 旧 | 新 |
  |---|---|
  | 行を合計の大きい順に並べ替え | 行を合計順に |
  | 列を合計の大きい順に並べ替え | 列を合計順に |
  | フェードインアニメーション | フェードイン |
  | 数値を省略表記（1.5M など） | 数値を省略表記 |

  **オプションのキー名・既定値・挙動は変更していない**（表示ラベルのみ）。
  ⚠ 編集パネルへの反映には splunkd の再起動が必要。

`.spl`: `dist/custom_viz_heat_matrix-1.1.4-<hash>.spl`
...
`.spl`: `dist/custom_viz_heat_matrix-1.1.3-2bb5ec3.spl`

### [1.1.2] - 2026-08-06

#### 変更

- **表示名から `Custom Viz ` プレフィックスを削除**（`Custom Viz Heat Matrix` → `Heat Matrix`）。
...
- `dist/custom_viz_heat_matrix-1.1.1-b4e4674.spl`
...
- `dist/custom_viz_heat_matrix-1.1.0-fcde869.spl`
