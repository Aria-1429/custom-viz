...
- **編集パネルのチェックボックスのラベルを短縮**した。長いラベルは編集パネルで
  ...
  `overflow: hidden`）。補足説明はラベルから外し、本 README に記載する方針に統一した。

  | 旧 | 新 |
  |---|---|
  | 自己ループ（送信元＝送信先）を表示 | 自己ループを表示 |
  | グラデーションリボン（送信元色→送信先色） | グラデーション |
  | 送信元を太く／送信先を細くする（方向を表現） | 方向で太さを変える |
  | 値でリボンを着色（低→高） | 値で着色 |
...
`.spl`: `dist/custom_viz_chord_flow-1.1.4-<hash>.spl`
...
`.spl`: `dist/custom_viz_chord_flow-1.1.3-2bb5ec3.spl`

### [1.1.2] - 2026-08-06

#### 変更

- **表示名から `Custom Viz ` プレフィックスを削除**（`Custom Viz Chord Flow` → `Chord Flow`）。
...
- `dist/custom_viz_chord_flow-1.1.1-b4e4674.spl`
...
- `dist/custom_viz_chord_flow-1.0.2-2dc2dde.spl`
...
#### パッケージ
- `dist/custom_viz_chord_flow-1.0.1-d824e00.spl`

### [1.0.0] - 2026-07-20

アニメーションパーティクル付きの有向コード・ダイアグラム（弦グラフ）。要素間の流量を弧で結んで可視化する。

#### 追加
- 新規作成（初回リリース）。
- パッケージ: `dist/custom_viz_chord_flow-1.0.0-beb3d05.spl`
