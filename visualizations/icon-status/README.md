...
- **編集パネルのチェックボックスのラベルを短縮**した。長いラベルは編集パネルで
  ...
  | 旧 | 新 |
  |---|---|
  | 省略表記（1.5K など） | 数値を省略表記 |

  **オプションのキー名・既定値・挙動は変更していない**（表示ラベルのみ）。
  ⚠ 編集パネルへの反映には splunkd の再起動が必要。

`.spl`: `dist/custom_viz_icon_status-1.1.6-<hash>.spl`
...
`.spl`: `dist/custom_viz_icon_status-1.1.5-2bb5ec3.spl`
...
- `dist/custom_viz_icon_status-1.1.3-b4e4674.spl`
