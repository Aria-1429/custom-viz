...
> - [apps/dpx/](apps/dpx/) … **DPX v1.2.0**。独立 React ページ上に自前エンジンを載せた
...
>   **背景エフェクト40種・パネル質感25種・出現アニメ12種**で見た目を作り込める（背景・パネル枠とも**手描き画材は canvas で実描画**）。
>   ダッシュボードは REST で動的に作成できる。**Mako 不使用**（同梱標準テンプレート1枚に SPA を集約）。
>   ホームから**複製・名前変更・JSON の書き出し／取り込み**ができ、新規作成時にテーマを選べる。<br>
...
>   編集は**あらゆる操作を Ctrl+Z で戻せ、戻しきると保存ボタンが押せなくなる**。<br>
>   **ネイティブ viz 20 種**（折れ線・棒・ランキング・ゲージ・進捗・ヒストグラム・ヒートマップ・
>   ドーナツ・単一値・状態・表・**マークダウン**・図形・コネクタ線ほか）を内蔵し、
...
>   （basic 構文のみ・オプション4個）より書ける。<br>**下の一覧のカスタム viz 30 種はすべて DPX 上でも動く**（iframe なし・Studio と同じ type）
