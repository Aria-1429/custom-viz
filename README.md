...
| <img src="visualizations/network-graph/examples/example.png" width="240"> | **[Network Graph](visualizations/network-graph/)**<br>v1.2.4 | 力学ベースのフォースダイレクテッド・ネットワーク図。送信元・宛先・値の**列を編集画面で選べ**、指定しなければ「名前らしい列2つ＋最も数値らしい列」を自動判定する（数値列が先頭にある並びでも正しく描ける）。反発計算に **Barnes-Hut 近似**を使い 300 ノードで約 21 倍高速、ノード上限は 400。**ノードのドラッグ中は反発・衝突が弱まり狙った場所に置ける**。流れる破線エッジ、線幅連動の矢印、ズーム／パン、**ノードのクリックでトークン設定**に対応。 |
