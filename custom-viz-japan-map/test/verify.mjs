// Japan Map viz のローカル検証（happy-dom、Splunk実機なし）
...
const BUNDLE = join(
    ...
    '..', 'dist', 'custom_viz_japanmap', 'visualization.js'
);
...
const ROWS = [
    ...
    ['33.59', '130.40', '35.68', '139.69', 'HIGH', '50', 'Fukuoka', 'Tokyo'],   // 大文字違い → high に合流
    ['26.21', '127.68', '35.68', '139.69', 'medium', '80', 'Naha', 'Tokyo'],
    ['38.27', '140.87', '35.68', '139.69', '', '10', 'Sendai', 'Tokyo'],        // 空severity → low
    ['35.18', '136.91', '35.68', '139.69', 'worm', '40', 'Nagoya', 'Tokyo'],    // 未知severity → extraColor1
    ['99.9', '135', '35.68', '139.69', 'high', '5', 'BadLat', 'Tokyo'],         // 緯度>90 → 除去
    ['abc', '135', '35.68', '139.69', 'low', '5', 'NaN', 'Tokyo'],              // 非数値 → 除去
];
// 有効な脅威 = 6 行
...
console.log('\n[1] initial render (dark, auto field detection)');
{
    ...
    check('prefecture borders drawn', doc.querySelectorAll('svg path[stroke="#5a9ae0"]').length > 10,
        `got ${doc.querySelectorAll('svg path[stroke="#5a9ae0"]').length}`);
    check('prefecture name tooltip present', titles().some((t) => t.includes('東京都') || t.includes('大阪府')),
        JSON.stringify(titles().filter((t) => /[぀-ヿ一-鿿]/.test(t)).slice(0, 3)));
    ...
    check('title shown', body.includes('JAPAN THREAT MAP'));
    check('legend/filter include worm severity', body.includes('worm'));
    check('arc tooltip src → dst', titles().some((t) => t.includes('Osaka') && t.includes('Tokyo')), JSON.stringify(titles().slice(0, 4)));
...
console.log('\n[3] display toggles + animation off');
{
    ...
    await sleep(250);
    const body = doc.body.textContent;
    check('title hidden', !body.includes('JAPAN THREAT MAP'));
...
console.log('\n[5] columnSelector DOS strings on renamed fields');
{
    ...
    check('names resolved via selector', titles().some((t) => t.includes('Osaka')));
}
