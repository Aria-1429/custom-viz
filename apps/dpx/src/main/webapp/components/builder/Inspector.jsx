...
import { BACKGROUND_OPTIONS } from '../renderer/BackgroundLayer';
import ColorRulesEditor from './ColorRulesEditor';
import { getDataSources, nextSourceId, panelsUsingSource } from '../data';
import { assignPanelToGroup, getGroups, groupOfPanel, nextGroupId, removeGroup } from '../renderer/groups';
import {
    ColumnMultiSelector,
    ColumnSelector,
    PresetSelector,
    SeriesColorsByFieldEditor,
    SeriesColorsEditor,
    StringListEditor,
    ThresholdEditor,
} from './optionEditors';
import { needsChoices, normalizeChoices } from '../data';
import SplEditor from './SplEditor';
import { usePanelFields } from '../viz/panelFields';
import TimeRangePicker from '../shared/TimeRangePicker';
// ⭐ **選択肢は Design Engine から取る**（ここで文字列を手書きしない）。
//   画材や動きを増やしたとき、この編集パネルを直し忘れて
//   「実装したのに選べない」になるのを防ぐ。
import {
    BRUSH_OPTIONS,
    DPX_PRESETS,
...
import { Button, ColorInput, Field, NumberInput, Section, Select, Slider, TextInput, Toggle, inputStyle, isTransparent } from '../shared/ui';
import { VIZ_CATEGORY_LABELS, defaultVariantFor, listViz, vizEditorConfig, vizOptionsSchema } from '../viz/registry';
...
const EDITOR_RENDERERS = {
    ...
    // ── 以下は現在どの viz も使っていないが、Studio 側に実在する型 ──────
    //   ⚠ **無い型を config に書くと Studio 実機では editorConfig 全体が消える**。
    //     DPX はここに無い型でも JSON で編集できるので壊れないが、
    //     同じ viz を Studio でも使うなら型の実在を必ず確かめること。
    //
    // 割合。⚠ **Studio では UI 値の 1/100 が options に届く**（5 → 0.05）。
    //   DPX でも同じ約束にしないと、同じ config を両方で使ったとき値がズレる。
    'editor.percent': (t, value, onChange) => (
        <Slider
            t={t}
    ...
    // フィールド名を 1 つ選ぶ。⚠ columnSelector と違い **DOS ではなく生の列名**が届く
    'editor.trellisSplitBy': (t, value, onChange, props, ctx) => (
        <Select
            t={t}
            value={value ?? ''}
            options={[{ value: '', label: '（なし）' }].concat(
                (ctx?.fields ?? []).map((f) => ({ value: f, label: f }))
            )}
            onChange={onChange}
        />
    ),
    // フィールド名 → 色。列が分かるので、選択肢から足せる形にする
    'editor.seriesColorsByField': (t, value, onChange, props, ctx) => (
        <SeriesColorsByFieldEditor t={t} value={value} onChange={onChange} fields={ctx?.fields ?? []} />
    ),
};
    ...
    if (!selectedPanel) {
        ...
        const dashDesign = (
                <Section t={t} title="デザイン">
                    ...
                    {/* ── Design Engine の 4 軸のうち Brush と Motion ────────
                        Theme（配色プリセット）と Surface（パネルの質感）は
                        それぞれ上とパネル側にある。ここは残りの 2 つ。 */}
                    <Field
                        t={t}
                        label="グラフの画材"
                        hint="線と塗りの質感。パネルの質感（面）とは別の軸です"
                    >
                        <Select
                            t={t}
                            value={definition.style?.brush ?? 'flat'}
                            options={BRUSH_OPTIONS}
                            onChange={(v) => patchDef({ style: { ...(definition.style ?? {}), brush: v } })}
                        />
                        {/* ⚠ **文字も歪む**（フィルタは描画結果に掛かるため）。
                            知らずに強くすると「ラベルが読めない」と誤解されるので、
                            選んだ時だけ理由と対処を出す。 */}
                        {definition.style?.brush && definition.style.brush !== 'flat' ? (
                            <div style={{ fontSize: 10, color: t.subColor, marginTop: 4, lineHeight: 1.5 }}>
                                ⚠ 画材は<b>文字の輪郭も揺らします</b>。読みにくいときは下の強さを下げてください。
                            </div>
                        ) : null}
                    </Field>
                    {/* 画材を選んだときだけ出す（flat では意味が無い） */}
                    {definition.style?.brush && definition.style.brush !== 'flat' ? (
                        <Field
                            t={t}
                            label={`画材の強さ（${Math.round((definition.style?.brushIntensity ?? 1) * 100)}%）`}
                            hint="小さいほど控えめ。文字が多い盤面では下げる"
                        >
                            <Slider
                                t={t}
                                min={0}
                                max={1}
                                step={0.05}
                                value={Number(definition.style?.brushIntensity ?? 1)}
                                onChange={(v) =>
                                    patchDef({ style: { ...(definition.style ?? {}), brushIntensity: v } })
                                }
                            />
                        </Field>
                    ) : null}
                    <Field
                        t={t}
                        label="動きの性格"
                        hint="出現アニメを個別指定している場合はそちらが優先されます"
                    >
                        <Select
                            t={t}
                            value={definition.style?.motion ?? 'subtle'}
                            options={MOTION_OPTIONS}
                            onChange={(v) => patchDef({ style: { ...(definition.style ?? {}), motion: v } })}
                        />
                    </Field>
                    ...
                    <Field
                        t={t}
                        label={`パネルの間隔（${definition.layout?.grid?.gap ?? 12}px）`}
                    ...
                    >
                        <Slider
                            ...
                            max={32}
                            step={1}
                            value={Number(definition.layout?.grid?.gap ?? 12)}
                            onChange={(v) => patchDef({ layout: { ...(definition.layout ?? {}), grid: { ...(definition.layout?.grid ?? {}), gap: v } } })}
                        />
                    </Field>
                    <Field
                        t={t}
                        label={`行の高さ（${definition.layout?.grid?.rowHeight ?? 72}px）`}
                        hint="グリッド1行分の高さ"
                    >
                        <Slider
                            t={t}
                            ...
                            max={140}
                            step={4}
                            value={Number(definition.layout?.grid?.rowHeight ?? 72)}
                            onChange={(v) => patchDef({ layout: { ...(definition.layout ?? {}), grid: { ...(definition.layout?.grid ?? {}), rowHeight: v } } })}
                        />
                    </Field>
                </Section>
        );
