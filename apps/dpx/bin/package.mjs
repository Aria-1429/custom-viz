...
//   のが典型的な事故で、実際 DPX は 45 件中 42 件が未記載のまま配布されていた
//   （2026-08-13 に発覚）。`visualizations/*` が既にこの指紋方式で守られていたので、
//   apps/* も同じ仕組みに合わせる。
{
    ...
    try {
        ...
        const statsPath = process.env.WEBPACK_STATS || '/tmp/dpx-stats.json';
        ...
        // ⚠ fail() は process.exit するのでここには来ない（＝本物の例外だけ）
