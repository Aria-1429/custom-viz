...
### 残件
- **fps の実測**（線 1 本 = 最大 2 path。大量パネルでの影響は未計測）
- Inspector に画材の選択 UI（現在は定義 JSON の `style.brush` で指定）
- **Motion Engine**（⚠ 既存 `ENTRANCE_ANIM` 12 種の**上の抽象**にする。
  置き換えると既存ボードの `entrance` 指定が無効になる）
