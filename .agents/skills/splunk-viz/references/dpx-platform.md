...
3. **白フラッシュ対策は3段**（(c) は 2026-08-15 追加）:
   (a) `pages/dpx/bootPaint.js`（評価されたら即座に暗転＋スプラッシュ。
   ⚠ 依存ゼロ・`!important` 必須）
   (b) DPX 内は SPA 遷移で再読込ゼロ（window マーカー生存で証明済み）
   **(c) ⭐ エントリを極小のシムにして本体を動的 import する**（§1.2）。
   これが**コールド入口の白の本丸**だった。
...
### 1.2 ⭐ コールド入口の白フラッシュ（**2026-08-15 に解決**・実機計測済み）

> ## 🛑【重要な訂正】「HTML 表示〜JS 開始の数百 ms は構造上塗れない」は**誤り**
>
> §1.1 の全数調査で「白フラッシュゼロの唯一のレバーは初期 HTML の `<head>` を
> 握ること＝Mako だけ」と結論していたが、**これは誤りだった**。
> 塗れなかったのではなく、**塗る処理を 4.88MB のバンドルの後ろに置いていた**のが原因。

**原因（実測。遷移開始を 0ms とする）**:

標準テンプレートは「ビュー名と同名の JS」を 1 本だけ読む。そこにアプリ全体
（**4.88MB / gzip 1.36MB**）を詰めていたため、`bootPaint` を**先頭の import** に
置いても、**モジュール評価はバンドル全体のダウンロード＋パースが終わるまで始まらない**。

| | 旧（単一バンドル） | 新（分割後） |
|---|---|---|
| HTML 応答完了 | 26ms | 26ms |
| `dpx.js` 取得 | 78〜603ms | **80〜83ms**（6.4KB） |
| **bootPaint 実行** | **800ms** | **85ms** |
| **first-paint** | **808ms** | **96ms** |
| 遷移直前の白（0.05s 刻み） | **8 フレーム（~0.8s）** | **1 フレーム（~0.05s）** |

→ HTML が来てから 800ms、ブラウザは**前のページを出したまま**だった。
Splunk の他ページは明るいので、これが「白フラッシュ」として見えていた。

**対策（3 点セット）**:

1. **エントリ `pages/dpx/index.jsx` を極小のシムにする**（6.4KB）。
   `bootPaint` を同期で実行し、アプリ本体は `import('./app')` で遅延読み込み。
   **テンプレートが読む JS は 1 本のままでよい**（同名 JS 規則を満たす）
2. **`__webpack_public_path__` を実行時に決める**。Splunk の静的 URL は
   `_bump` のたびに変わるキャッシュキー（`/static/@<hash>/app/dpx/pages/`）を含むので、
   **ビルド時に固定できない**。自分自身の `<script src>` からディレクトリを逆算する
3. **チャンク名を固定する**（`chunkFilename: '[name].chunk.js'` ＋ `chunkIds:'named'` ＋
   vendor 分割の停止）。⚠ 既定の数値 ID（`323.chunk.js`）は**モジュールグラフが変わると
   番号がずれる**うえ、`appserver/static` は上書きインストールで**古いファイルが消えない**
   ので、孤児のチャンクが積み上がる

**⚠ 残る 1 フレーム（~0.05s）は消せない**（未検証だが機序は明確）:
HTML 応答（26ms）〜 `dpx.js` 評価（85ms）の間はまだ何も塗れないので、
ブラウザは前のページを表示し続ける。ここを消すには初期 HTML の `<head>` が要る＝
§1.1 の Mako の話に戻る。**60ms 弱なので実用上は問題にならない**と判断した。

**教訓**:
- **「構造上できない」は、自分の実装を除外してから言う。** 今回は
  「テンプレートが 1 本しか読まない」を「1 本に全部入れるしかない」と読み違えていた。
  **1 本のサイズは自分で決められた**
- 白の原因は**推測せず timeline で切り分ける**（`first-paint` と
  「自前の処理が走った時刻」を突き合わせると一発で分かる）
...
> ## 🛑【訂正・2026-08-15】この節の「白フラッシュは Mako でしか消せない」は**誤り**
>
> 当時の結論は **「①Mako 全廃 ②URL 維持 ③REST 動的作成 ④白フラッシュゼロ は同時に
> 満たせない」**「白フラッシュゼロの唯一のレバーは初期 HTML の `<head>`＝Mako だけ」
> だったが、**④は Mako 無しでほぼ達成できた**（0.8s → 0.05s。実測は §1.2）。
>
> **誤りの原因**：「テンプレートが読む JS は 1 本」という制約を
> 「その 1 本に全部入れるしかない」と読み違えていた。
> **1 本のサイズは自分で決められる**——エントリを 6.4KB のシムにして
> 本体を動的 import すれば、暗転は 85ms で走る。
>
> ⚠ 厳密には HTML 応答（26ms）〜 JS 評価（85ms）の **1 フレーム（~0.05s）は残る**。
> ここを消すには `<head>` が要るので「ゼロ」ではないが、実用上は問題にならない。
>
> **この節の Mako 側の調査（逃げ道の全数調査・AppInspect）は今も有効**。
> 訂正したのは「白フラッシュの唯一のレバー」という部分だけ。

**当時の結論**：Mako 継続が正解（`deactivate_custom_mako_templates` が `false` である限り動く）。
→ **現在は Mako 全廃済み**（v0.2.0）で、白フラッシュも §1.2 の分割で解決した。
**Splunk のバージョンを上げるときは非推奨ページを確認する**（設定自体はもう使っていない）。
...
- **配色プリセット18種**（`themes.js`）。系列色パレットもプリセットごとに持つ（`useDpxTheme().palette`）:
  ...
  | `watercolor` | **水彩**。水彩紙の生成り地に顔料のウォッシュ。乾いた縁が濃い「エッジの濃まり」。丸ゴシック |
  | `crayon` | **クレヨン／オイルパステル**。黒画用紙＝手描き系で唯一のダーク。紙の目＋こすった跡 |
  | `pencil` | **色鉛筆**。スケッチブックの紙目。青鉛筆がアクセント・赤鉛筆がエラー |
  | `inkwash` | **インク＋水彩（ペン画）**。旅帳の生成り＋群青のウォッシュ＋セピアインクの文字 |
  | `liquidGlass` | **Liquid Glass（iOS 26）**。WWDC25 の銀地＋細グリッド＋色の溜まり。Apple システムカラー系・SF Pro 系フォント |
  ...
  - **CSS で作れないもの**: `repeating-linear-gradient` と `box-shadow` は
    ...
    **原理的に表現できない**。当初の実装はこれを CSS でやろうとして
...
| `watercolor` | **水彩**。輪郭線なし・縁に顔料が溜まる inset＋ウォッシュ。角丸9px を自前で持つ |
| `crayon` | **クレヨン**。4px の蝋の線（半透明）＋ずらした二度塗り。角丸12px |
| `pencil` | **色鉛筆**。2方向クロスハッチ（115deg 密＋25deg 粗）＋二度引きの輪郭線 |
| `inkwash` | **インク＋水彩**。2px のインク線＋線からはみ出すウォッシュ＋右下にペンの溜まり |
| `liquidGlass` | **Liquid Glass**。**背景が縁で実際に屈折する**＝SVG 変位マップを `backdrop-filter: url(#dpx-liquid-lens)` で適用（⭐ **同一ドキュメント参照なので DPX でだけ成立**。iframe の Studio 拡張には持ち出せない。**Chromium 実機で屈折を確認済み**。定義は `liquidGlassDefs.jsx`、DpxDashboard が常設）。**色収差つき**＝R/G/B を別 scale で変位→加算合成（⚠ **scale 差を 20 にしたら端に実在しない青緑の帯**が出た。バックドロップ端のクランプ画素がチャンネルごとに別方向へずれるため。**差は 10 前後まで**。実機で確認）。＋スペキュラ／明縁／反射ストリーク／下辺の分光／厚みの影。角丸24px を自前で持つ。⚠ **ダークテーマは照りゼロ＋blur/saturateなし（url のみ）**＝微弱な白い照りや 2px のブラーでも暗い地では「霧状の滲み」に見える（実機で2回指摘されて確定。ライトだけ照りを持つ）。屈折帯は各辺20%に絞る（遷移が広いと中央まで滲む）。⚠ backdrop-filter を使うので**動く背景と重ねない**（毎フレーム再ブラー） |
...
- **背景エフェクト40種**（`BackgroundLayer.jsx` の `BACKGROUND_OPTIONS`。v0.1.0 で 23→40）:
  ...
  **meteor / radar / bubbles / snow / fireflies**、
  ...
  グラデ系＝glow / aurora / vignette / thermalScan / **washBlooms（水彩のにじみ）** /
...
- ⚠ **ヘッダを隠すなら「戻る導線」を自前で用意する**（`SplunkHomeLink.jsx`）。
...
| ⭐ **書き込むタイミングを絞る** | **これが本筋**。ドラッグ中の座標も打鍵中の文字列も「**一時的な表示状態**」であって保存対象ではない。中間状態を定義に書くから履歴が埋まり、それを「まとめキー」で無かったことにする対症療法が要っていた。**確定時に1回だけ書く**ようにすれば、**JSON の変化＝1操作**が自然に成立する。ドラッグ＝`pointerup`、テキスト欄＝`blur`/Enter |
...
**テキスト欄の確定**（`TextInput` / `engine/ui.jsx`）:
...
**複数のパネルを1つの区画としてくくり、区画名を与える**（2026-08-12 実機確認済み）。
...
（2026-08-12 実機確認済み）。時間範囲入力のトークンを書き換えるので、
...
| **`onOptionsChange` が空実装だった** | `() => {}` が渡っていたため、viz からの書き戻しが**黙って捨てられる**。viz 側は保存された前提で描き続けるので**「動くのに保存されない」** | `onPatchPanel(panel.id, { options: {...} })` に接続（修正済み） |
...
| `dpx.gauge` | ゲージ（達成率） | 1列目=値（2列目=目標値・任意） | **上限に対する到達度**を弧で見せる。3/4円 / 半円、％表示、カウントアップ。単一値と違い「満タン」が意味を持つ指標向け（SLO・使用率） |
| `dpx.progress` | 進捗バー（目標比） | 1列目=ラベル、2列目=値（3列目=目標値・任意） | **項目ごとに目標が違ってよい**（ランキングは相対比較、こちらは絶対達成率）。100%超は色で警告し、**数字は実値のまま**出す |
| `dpx.histogram` | ヒストグラム（分布） | 1列目=数値の並び | **ラベル列が要らない**。階級分けは viz 側（スタージェスの公式で自動）＝**画面で切り方を変えられる**。ばらつきを見る用途 |
| `dpx.heatmap` | ヒートマップ（クロス集計） | 1列目=行、2列目=列、3列目=値 | 「曜日×時間帯」等の**2軸の交差**で濃淡。ホバーで詳細、クリックでトークン。**欠測は枠だけ**（0 と区別する） |
...
| `dpx.sankey` | フロー図（サンキー） | 3列=src,dst,値／**4列以上=多段** | ⭐**標準 `splunk.sankey` は 4 列以上の 3 列目以降を黙って捨てる**（実機確認）。こちらは全段描く。**ホバーで経路全体を追跡**・**段ごとの離脱を数値表示**・上位Nは「その他」へ畳む（切り捨てない） |
| `dpx.linkGraph` | 関係図（ネットワーク） | 3列=src,dst,値 | ⚠標準 `splunk.networkGraph` は**力学配置も値による色分けも標準でできる**ので、差別化は**標準にできないこと**だけ: ⭐**線の太さが値に連動**・⭐**ホバー強調＋ツールチップ**・⭐**流れのアニメ**。配置は**乱数なし**（毎回同じ絵） |
| `deco.text` | テキスト | 不要 | `$トークン$` 展開・グロー |
| `deco.clock` | 時計 | 不要 | ライブ更新 |



### 7.0 標準 viz のカタログ（実機で見る）

**`apps/dpx/examples/native-viz-catalog.json`** … ネイティブ 22 種を 4 タブに並べた一覧。
パネルのタイトルに **viz キーとデータ規約**が入っているので、
「どれを使うか」を実物を見て決められる。実機へ入れるのは:

```bash
node tools/dashboard-loop/src/dp-push.mjs \
     apps/dpx/examples/native-viz-catalog.json search dpx_viz_catalog "DPX 標準 viz カタログ"
```

- **`apps/dpx/tools/check-catalog.mjs`** が `yarn test` で
  **viz キー・オプション名・データソース参照・網羅**を検査する。
  **viz を足してカタログに載せ忘れるとテストが落ちる**（一覧が腐らない）
- ⚠ **`shape.*` / `deco.*` / `dpx.linkLine` は既定 `frameless`＝タイトルバーが出ない**
  （`defaultVariantFor()`）。カタログのように**名前を読ませたい用途では
  `style.variant: 'noc'` を明示する**。`style` を消しても既定に落ちるだけで直らない（実機で踏んだ）

---

### 7.1 ネイティブ viz を足すときに踏んだ罠（2026-08-15）

- ⚠⚠ **HTML を描く viz は「親に color」だけでは足りない。**
  Splunk のページ側に **`h1, p { color: #fff }` のインライン CSS** があり
  （DPX の外・変更できない）、`.dpx-md` にだけ色を置くと **`<p>` だけ白のまま**。
  ライト系テーマで**本文が消える**（太字だけ読めるので「行が重なっている」ように
  見えて誤診した）。→ `p / li / td / span / div` を列挙して明示的に上書きする。
  **computed style を実機で読む**まで原因は分からなかった（DOM の座標は正常だった）
- ⚠ **テンプレートリテラルの中にバッククォートを書かない**（コメントでも）。
  文字列がそこで終わり `Missing semicolon` になる（CSS を JS 内に書くときに踏んだ）
- **画材（Brush）は全 viz に入れない。** 図形が主役の viz（棒・弧・面）だけ。
  **文字を読ませる viz**（table / value / status / markdown）と
  **セルが小さい viz**（heatmap）は、線が乗ると値が読めなくなる

- ⚠⚠ **空データでも `d.column(0)` が呼べる前提で書く。**
  viz は**フックのルール**により `useMemo` を early return より前に置くので、
  **データ未着でも列アクセスが走るのが正常**。以前は `EMPTY_VIZ_DATA` に
  `column()` が無く、**`column is not a function` で 3 つの viz が同時に落ちた**
  （実機の `dp-allviz-check.mjs` が検出）。→ 空の戻り値にも**同じメソッドを生やす**。
- ⚠⚠ **区切り文字に生の NUL を書かない。** ヒートマップの Map キーで
  `\x00` を使い、**ファイルがバイナリ判定されて grep が全滅**した
  （`layers.test.mjs` が検出。sankey-flow と同じ轍）。可読の `␟` を使う。
  空白や `-` も不可（「"a b"×"c"」と「"a"×"b c"」が同じキーに潰れる）。
- **計算は `.js`（依存ゼロ）に切り出してテストする。** 階級分け・クロス集計は
  `viz/aggregate.js` に置き `test/nativeViz.test.mjs` で固定した。
  **境界（全部同値＝0除算・最大値の帰属・欠測）は目視で判定できない。**
- `pickTextColor()` は**地の色が解釈できないと null を返す**。`?? t.titleColor` で必ず受ける。
- 円弧は `stroke` で描く（`fill` ではない）。⚠ **360度ちょうどの弧は描けない**（長さ0と解釈される）。

- ⚠⚠ **`width` プロップを SVG 幅にそのまま使わない。** プロップは既定値のことがあり、
  **パネルの 3 割しか使わない絵**になる（サンキーで実際に発生）。
  他の viz と同じく **`useContainerSize()` で実測する**。
  ⚠ このとき **early return の側にも計測用の要素を描く**こと。
  ref が付いた要素が DOM に出ないと幅が測れず、
  **「描画領域が狭すぎます」から永久に抜けられないデッドロック**になる。
- ⚠ **SVG の `marker` は既定で `strokeWidth` 基準**（`markerUnits="strokeWidth"`）。
  線の太さを値に連動させると**矢印まで比例して巨大化**する
  （9px の線に 45px の矢印が付いて絵が潰れた）。
  向きが分かればよいので **`markerUnits="userSpaceOnUse"`** で一定にする。
- ⚠⚠ **「途中で終わるデータ」を不正扱いして捨てない。**
  多段フローで末尾の段が空の行（離脱・タイムアウト・遮断）を
  「空欄がある＝不正」として行ごと落としていた。結果:
  **離脱ぶんの量がグラフから消え**、さらに目玉機能の「段ごとの損失」が
  **常に 0 になって死んでいた**（カタログの実データで 255 件が消えた）。
  → **最初の空欄で打ち切って残す**。落とすのは出発点しか無い行だけ。
  **「絵は出ているのに機能が動かない」ときは、入力の捨て方を疑う。**
- ⚠ **アニメを足すときは既存の keyframes を使う**（`parts.jsx` の `dpxLinkFlow` 等）。
  動かしてよいのは **`stroke-dashoffset` / `transform` / `opacity`** だけ
  （合成に載る）。独自に足すと面積比例の再描画を招きやすい。

#### 実機で確かめるときのセレクタ（撮影スクリプト）

- ⚠ **タブの切り替えは `button` で探す。** `getByText('フロー・関係')` だと
  **パネルの見出し文字に先に当たり**、タブが切り替わらないまま撮れてしまう
  （「実装が動いていない」と誤診しかけた）。`page.locator('button', {hasText: …})` を使う。
- ⚠ **パネルは `[data-panel-id="<id>"]` で名指しする。** 「rect が 8 個以上の svg」の
  ような形での特定は**別のパネルに当たる**（実際に空の svg を掴んで
  「ホバーが効かない」と誤診した）。
- **ホバーの検証は `opacity` を数える。** 「光った気がする」は判定にならない。
  経路追跡なら**薄くなったリンクの本数**が期待値と一致することまで見る
  （例: 9 本中 4 本が 0.15 になる）。
...
**残件**:
- ~~undo/redo~~ … **完了**（あらゆる操作を Ctrl+Z で戻せる／戻しきると保存不可。§6.8.6b）
- パネルの重なり検知、複数選択
